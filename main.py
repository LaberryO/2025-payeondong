import os, pygame, pygame.locals, sys, time, random, pymunk, pymunk.pygame_util;   
...
class Game:
    def __init__(self):
        ...
        self.drawOptions = pymunk.pygame_util.DrawOptions(self.display)

        self.floor = pymunk.Body(body_type=pymunk.Body.STATIC)
        self.floor.position = (0, self.screen.height - 64)
        self.floorShape = pymunk.Segment(self.floor, (0, 0), (self.screen.width, 0), 1)

        self.space.add(self.floor, self.floorShape)
    ...
    def reset(self):
        ...
        self.player = Entity.Player(self.screen, self.images, self.space)
        if self.fullscreen:
    ...
    def game(self):
        ...
        self.player.move(self.deltaTime, pressedKeys)
